...
export const Round4LegalBattlePage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Banner */}
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-navy-900/60 p-6 rounded-2xl border border-neutral-800/80 backdrop-blur-sm">
        <div className="space-y-1.5">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
              <Gavel className="w-7 h-7 text-primary-400" />
          ...
          <p className="text-sm text-neutral-400">
            8 finalist squads randomly paired into 4 fictional legal matchups. Case preparation, oral hearings, faculty judging rubric, and secret agent submissions.
          </p>
        </div>

        {/* Global Round Action Bar */}
        <div className="flex items-center gap-2.5 flex-wrap">
          {!config.isFinalized && !config.pairingsConfirmed && (
            ...
            >
              <ArrowUpDown className="w-4 h-4 mr-1.5 text-primary-400" />
              Randomize Pairings
            </Button>
          )}
          ...
          <Button
            ...
            className="border-neutral-700 hover:bg-neutral-800"
          >
            <Settings className="w-4 h-4 mr-1.5 text-neutral-400" />
            Rules & Rubric
          </Button>
          ...
          {!config.isFinalized && (
            <>
              <Button
                ...
                className="border-primary-800/60 hover:bg-primary-950/40 text-primary-300"
              >
      ...
      {!round3Finalized && (
        <div className="bg-amber-950/30 border border-amber-800/60 rounded-xl p-4 flex items-center justify-between gap-4">
          ...
          <Link to="/round-3">
            <Button variant="secondary" size="sm" className="border-amber-700 text-amber-200 hover:bg-amber-900/40">
      ...
      {!config.isFinalized && !config.pairingsConfirmed && (
        <div className="bg-yellow-950/25 border border-yellow-800/50 rounded-xl p-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="sm"
            onClick={() => setIsConfirmPairingsOpen(true)}
            className="border-yellow-700 text-yellow-200 hover:bg-yellow-900/40 shrink-0"
          >
      ...
      {!config.finalScoreFormula.isFormulaConfirmed && (
        <div className="bg-primary-950/30 border border-primary-800/50 rounded-xl p-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="sm"
            onClick={() => setIsRulesModalOpen(true)}
            className="border-primary-700 text-primary-200 hover:bg-primary-900/40 shrink-0"
          >
            Review Formula
          </Button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5">
        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Finalist Squads</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.eligibleTeamsCount}</span>
            <span className="text-xs text-neutral-500">/ 8 required</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">
            {stats.eligibleTeamsCount === 8 ? 'Complete squad roster' : 'Roster incomplete'}
          </p>
        </Card>

        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Matchup Pairs</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.pairsConfiguredCount}</span>
            <span className="text-xs text-neutral-500">/ 4 pairs</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">
            {stats.pairingsConfirmed ? (
              <span className="text-emerald-400 font-medium">Locked & Confirmed</span>
            ) : (
              <span className="text-amber-400 font-medium">Unconfirmed</span>
            )}
          </p>
        </Card>

        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Cases Assigned</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.casesAssignedCount}</span>
            <span className="text-xs text-neutral-500">/ 4 cases</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">Counsel sides designated</p>
        </Card>

        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Hearings & Exchange</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.hearing2CompletedCount}</span>
            <span className="text-xs text-neutral-500">/ 4 pairs done</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">
            Exchange: {stats.fileExchangeCompletedCount}/4 complete
          </p>
        </Card>

        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Faculty Scorecards</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.judgingCompletedCount}</span>
            <span className="text-xs text-neutral-500">/ 8 squads</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">100-mark rubric evaluated</p>
        </Card>

        <Card className="bg-navy-900/50 border-neutral-800/70 p-4">
          <p className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Agent Guess Verified</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{stats.agentGuessesVerifiedCount}</span>
            <span className="text-xs text-neutral-500">/ 8 squads</span>
          </div>
          <p className="text-[11px] text-neutral-500 mt-1">Restricted portal verified</p>
        </Card>
      </div>

      {/* Finalization Checklist Drawer */}
      <Card className="bg-navy-900/40 border-neutral-800/80 overflow-hidden">
        <button
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-sm font-semibold text-white">Pre-Finalization Safeguards & Diagnostics</span>
          ...
          <div className="flex items-center gap-2">
            ...
            {showChecklist ? <ChevronUp className="w-4 h-4 text-neutral-400" /> : <ChevronDown className="w-4 h-4 text-neutral-400" />}
          </div>
        </button>

        {showChecklist && (
          <div className="p-4 pt-0 border-t border-neutral-800/60 grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
            {engine.checklist.map((item) => (
              ...
              >
                ...
                <div>
                  ...
                  {item.details && <p className="text-neutral-400 mt-0.5">{item.details}</p>}
                </div>
              </div>
            ))}
          </div>

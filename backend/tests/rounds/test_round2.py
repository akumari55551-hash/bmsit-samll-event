...
def test_cabo_api_blocks_unfinalized_round1(client):
    # Attempting to submit game placements before Round 1 is finalized
    payload = {
        "placements": [{"team_id": "team-1", "placement": 1}]
    }
    res = client.post("/api/rounds/2/games?game_number=1", json=payload)
    assert res.status_code == 400
    assert "Round 1 is not yet finalized" in res.json()["detail"]

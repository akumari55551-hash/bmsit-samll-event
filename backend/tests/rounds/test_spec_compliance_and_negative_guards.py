...
def test_unauthorized_finalization(client):
    # Attempt finalization with non-organizer, non-admin role (e.g. participant or judge)
    headers = {'X-Operator-Role': 'participant', 'X-Operator-Id': 'student-1'}
    res = client.post('/api/rounds/1/finalize', headers=headers)
    assert res.status_code == 403
    assert 'Operation restricted to roles: organizer, admin' in res.json()['detail']

    res2 = client.post('/api/rounds/2/finalize', headers=headers)
    assert res2.status_code == 403

    res3 = client.post('/api/rounds/3/finalize', headers=headers)
    assert res3.status_code == 403

    res4 = client.post('/api/rounds/4/finalize', headers=headers)
    assert res4.status_code == 403

    res5 = client.post('/api/rounds/finale/finalize', headers=headers)
    assert res5.status_code == 403

def test_invalid_score_boundaries(client):
...
def test_invalid_transaction_reversals(client):
    # Attempt to reverse non-existent transaction
    res = client.post('/api/rounds/3/transactions/rev-non-existent/reverse', json={'reason': 'Error'})
    assert res.status_code == 404

def test_non_qualified_team_rejected_in_r2(client):
    ...
    client.post('/api/rounds/1/finalize')
    ...
    assert res.status_code == 400
    assert 'not qualified from Round 1' in res.json()['detail']

def test_duplicate_finalization_idempotency(client, db_session):
...
def test_role_based_access_controls(client):
    # 1. Scorekeeper can record timing
    res_sk = client.post(
        '/api/rounds/1/teams/team-1/timings',
        json={'mini_round_number': 1, 'hints_used': 0},
        headers={'X-Operator-Id': 'sk-1', 'X-Operator-Role': 'scorekeeper'}
    )
    assert res_sk.status_code == 200

    # But scorekeeper CANNOT finalize Round 1
    ...
    res_j_fin = client.post(
        ...
        headers={'X-Operator-Id': 'judge-1', 'X-Operator-Role': 'judge'}
    )
    assert res_j_fin.status_code == 403

    # 3. Lead judge CAN resolve a tie review
    res_lj = client.post(
        '/api/ties/review/tie-nonexistent/resolve',
        json={'decision': 'FAVOR_TEAM', 'advancing_team_ids': ['team-1']},
        headers={'X-Operator-Id': 'lj-1', 'X-Operator-Role': 'lead_judge'}
    )
    # Either 404 (because tie does not exist) or 200; NOT 403 forbidden!
    assert res_lj.status_code != 403

    # 4. Admin has universal access
    res_admin = client.post(
        '/api/rounds/1/finalize',
        headers={'X-Operator-Id': 'admin-1', 'X-Operator-Role': 'admin'}
    )
    # Returns 200 (finalization logic response) - definitely not 403
    assert res_admin.status_code == 200

def test_production_auth_requirement(client, monkeypatch):
    ...
    assert 'credentials required' in res.json()['detail']

    # With Bearer token, request proceeds
    res_auth = client.post(
        '/api/rounds/1/finalize',
        headers={'Authorization': 'Bearer admin-token', 'X-Operator-Role': 'admin', 'X-Operator-Id': 'admin-super'}
    )

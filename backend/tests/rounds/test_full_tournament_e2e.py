...
from app.schemas.rounds.round4 import CreatePairInput, UpdateStageInput, SubmitJudgeScoreInput
from app.schemas.rounds.finale import SubmitScorecardInput

marshal = AuthenticatedUser(id="marshal-lead", username="Marshal Lead", email="marshal@bmsit.in", role="marshal")
organizer = AuthenticatedUser(id="org-lead", username="Organizer Lead", email="org@bmsit.in", role="organizer")
judge = AuthenticatedUser(id="judge-lead", username="Judge Lead", email="judge@bmsit.in", role="judge")

def test_full_tournament_e2e_flow(client, db_session):
    """
    ...
    r1_check = client.get("/api/rounds/1/qualification").json()["data"]
    assert r1_check["can_finalize"] is True
    assert r1_check["completed_count"] == 32

    r1_fin = client.post("/api/rounds/1/finalize").json()["data"]
    ...
    for g_num in [1, 2, 3]:
        ...
        res = client.post(f"/api/rounds/2/games?game_number={g_num}", json={"placements": placements})
        assert res.status_code == 200

    r2_check = client.get("/api/rounds/2/qualification").json()["data"]
    assert r2_check["can_finalize"] is True

    r2_fin = client.post("/api/rounds/2/finalize").json()["data"]
    ...
    client.put("/api/rounds/3/config", json={"is_scoring_configured": True})
    ...
    r3_check = client.get("/api/rounds/3/qualification").json()["data"]
    assert r3_check["can_finalize"] is True

    r3_fin = client.post("/api/rounds/3/finalize").json()["data"]
    ...
    # Confirm pairings
    client.post("/api/rounds/4/pairs/confirm")
    ...
    r4_check = client.get("/api/rounds/4/qualification").json()["data"]
    assert r4_check["can_finalize"] is True

    r4_fin = client.post("/api/rounds/4/finalize").json()["data"]
    ...
    # -------------------------------------------------------------
    # Confirm rules
    client.put("/api/rounds/finale/config", json={"is_scoring_rules_confirmed": True})
    ...
    fin_check = client.get("/api/rounds/finale/qualification").json()["data"]
    assert fin_check["can_finalize"] is True

    finale_fin = client.post("/api/rounds/finale/finalize").json()["data"]
    assert finale_fin["finalized"] is True
    assert finale_fin["champion_team_id"] == "team-1"

    # Verify Leaderboard shows Podium Honors
    leaderboard = client.get("/api/rounds/finale/leaderboard").json()["data"]
    ...
    audit_res = client.get("/api/audit/logs?limit=100")

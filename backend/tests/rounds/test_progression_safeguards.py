...
marshal_user = AuthenticatedUser(id="marshal-1", username="Marshal 1", email="m1@bmsit.in", role="marshal")
organizer_user = AuthenticatedUser(id="lead-org", username="Organizer", email="org@bmsit.in", role="organizer")

def test_cross_round_gating(client, db_session):
    ...
    r2_res = client.post("/api/rounds/2/games?game_number=1", json=r2_payload)
    ...
    r3_res = client.post("/api/rounds/3/teams/team-1/transactions", json=r3_payload)
    ...
    r4_res = client.post("/api/rounds/4/judging/team-1/scores", json=r4_payload)
...
def test_tie_review_resolution_workflow(client, db_session):
    ...
    res_tie = client.post(f"/api/ties/review/{tie_record['id']}/resolve", json=resolve_payload)

...
def reverse_transaction(
    ...
    reversal_tx = BlackMarketTransactionModel(
        ...
        notes=f"Reversed by {actor.username} ({actor.role})"
    )
    db.add(reversal_tx)
...
def finalize_round3(db: Session, actor) -> Dict[str, Any]:
    cfg = get_or_create_black_market_config(db)
    if cfg.is_finalized:
        return {"can_finalize": True, "issues": [], "finalized": True, "message": "Round 3 is already finalized."}

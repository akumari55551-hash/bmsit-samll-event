...
def ensure_round4_pairs(db: Session):
    for i in range(1, 5):
        pid = f"pair-{i}"
        p = db.query(Round4PairModel).filter(Round4PairModel.id == pid).first()
        if not p:
            p = Round4PairModel(
                id=pid,
                pair_number=i,
        ...
        for s_id in STAGE_IDS:
            ...
            if not st:
                st = Round4StageTimingModel(id=stid, pair_id=pid, stage_id=s_id, status="not_started")
                db.add(st)
    db.commit()

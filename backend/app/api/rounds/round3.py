...
def update_config(
    ...
    actor: AuthenticatedUser = Depends(require_role(["organizer", "admin"]))
):
...
def record_transaction(
    ...
    db: Session = Depends(get_db),
    actor: AuthenticatedUser = Depends(require_role(["organizer", "marshal", "scorekeeper", "admin"]))
):
...
def reverse_transaction_api(
    ...
    rev = round3_service.reverse_transaction(db, transaction_id, payload.reason, actor)
    res = TransactionResponse(
        id=rev.id,
        team_id=rev.team_id,
        amount=rev.amount,
...
def add_code_fragment_api(
    ...
    db: Session = Depends(get_db),
    actor: AuthenticatedUser = Depends(require_role(["organizer", "marshal", "admin"]))
):
...
def finalize_round3(
    db: Session = Depends(get_db),
    actor: AuthenticatedUser = Depends(require_role(["organizer", "admin", "lead_judge"]))
):
    """Officially seal Round 3 results and advance 8 squads to Round 4: The Legal Battle."""
    res = round3_service.finalize_round3(db, actor)
